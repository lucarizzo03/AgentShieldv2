...
- Tests: `redis-server --daemonize yes --port 6379 && APP_ENV=dev .venv/bin/python -m pytest -q --ignore=tests/e2e`
  (`tests/e2e` and the Claude-backed tests in `tests/unit/test_engine.py` need a live server / `ANTHROPIC_API_KEY`.)
